...
public static class RecycleInDetailEndpoints
{
    public static IEndpointRouteBuilder MapRecycleInDetailEndpoints(this IEndpointRouteBuilder app)
    {
        RouteGroupBuilder group = app.MapGroup("/api/recycle-in-details")
            .WithTags("Recycle In Details");

        group.MapGet("", async (RecycleInDetailRepository repository) =>
        {
            return await ApiResponseHelper.HandleAsync(repository.GetAllAsync);
        });

        group.MapGet("{id:int}", async (RecycleInDetailRepository repository, int id) =>
        {
            return await ApiResponseHelper.HandleAsync(() => repository.GetByIdAsync(id));
        });

        group.MapGet("by-recycle-in/{recycleInId:int}", async (RecycleInDetailRepository repository, int recycleInId) =>
        {
            return await ApiResponseHelper.HandleAsync(() => repository.GetByRecycleInIdAsync(recycleInId));
        });

        group.MapPost("", async (RecycleInDetailRepository repository, Dictionary<string, JsonElement> body) =>
        {
        ...
        group.MapPut("{id:int}", async (RecycleInDetailRepository repository, int id, Dictionary<string, JsonElement> body) =>
        {
        ...
        group.MapPut("by-recycle-in/{recycleInId:int}/bulk", async (RecycleInDetailRepository repository, int recycleInId, Dictionary<string, JsonElement> body) =>
        {
            ...
            return await ApiResponseHelper.HandleAsync(() => repository.BulkSaveAsync(recycleInId, userId, details.GetRawText()));
        });

        group.MapDelete("{id:int}", async (RecycleInDetailRepository repository, int id, int userId) =>
        {
    ...
    private static SqlParameter[] BuildUpsertParameters(Dictionary<string, JsonElement> body)
    {
        ...
        [
            ...
            SqlParameterHelper.Int("RecycleInId", body),
