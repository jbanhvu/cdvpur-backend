...
public static class MaterialTypeEndpoints
{
    public static IEndpointRouteBuilder MapMaterialTypeEndpoints(this IEndpointRouteBuilder app)
    {
        RouteGroupBuilder group = app.MapGroup("/api/material-types")
            .WithTags("Material Types");

        group.MapGet("", async (MaterialTypeRepository repository) =>
        {
            return await ApiResponseHelper.HandleAsync(repository.GetAllAsync);
        });

        group.MapGet("{id:int}", async (MaterialTypeRepository repository, int id) =>
        {
            return await ApiResponseHelper.HandleAsync(() => repository.GetByIdAsync(id));
        });

        group.MapPost("", async (MaterialTypeRepository repository, Dictionary<string, JsonElement> body) =>
        {
        ...
        group.MapPut("{id:int}", async (MaterialTypeRepository repository, int id, Dictionary<string, JsonElement> body) =>
        {
        ...
        group.MapDelete("{id:int}", async (MaterialTypeRepository repository, int id, int userId) =>
        {
