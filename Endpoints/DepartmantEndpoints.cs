...
public static class DepartmantEndpoints
{
    public static IEndpointRouteBuilder MapDepartmantEndpoints(this IEndpointRouteBuilder app)
    {
        RouteGroupBuilder group = app.MapGroup("/api/departmants")
            .WithTags("Departmants");

        group.MapGet("", async (DepartmantRepository repository) =>
        {
            return await ApiResponseHelper.HandleAsync(repository.GetAllAsync);
        });

        group.MapGet("{id:int}", async (DepartmantRepository repository, int id) =>
        {
            return await ApiResponseHelper.HandleAsync(() => repository.GetByIdAsync(id));
        });

        group.MapPost("", async (DepartmantRepository repository, Dictionary<string, JsonElement> body) =>
        {
        ...
        group.MapPut("{id:int}", async (DepartmantRepository repository, int id, Dictionary<string, JsonElement> body) =>
        {
        ...
        group.MapDelete("{id:int}", async (DepartmantRepository repository, int id, int userId) =>
        {
